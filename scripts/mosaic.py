...
"""Build monolithic per-year BII COGs from the per-chunk tile outputs.
    ...
    python scripts/mosaic.py --year 2020 --year 2021 # only these years
...
def main(argv=None) -> dict:
    ...
    parser.add_argument("--year", type=int, action="append", help="only these years (repeatable; default: all)")
    parser.add_argument("--overwrite", action="store_true", help="rebuild years whose output already exists")
    args = parser.parse_args(argv)

    result = mosaic.run(years=args.year, run_id=args.run_id, overwrite=args.overwrite)
