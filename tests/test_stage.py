...
Batch client and ``docker run`` are fakes recording their inputs; worker dispatch runs against a
stub dataset module injected into ``MODULES``.
"""
...
def test_stage_dispatches_to_module_by_manifest_line(stub):
    stage.stage(stage.manifest_items("fake")[1])
...
def test_pending_skips_unit_whose_empty_marker_exists(local_roots):
    ...
    io.put_bytes(b"", dst1 + stage.EMPTY_MARKER)  # u1 produced nothing last run
...
def test_stage_reads_the_indexed_unit_from_env(stub, local_roots, monkeypatch):
    ...
    stage.stage()  # no arg -> reads the env-indexed line
